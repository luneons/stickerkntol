...
START_TEXT = """
Hey! aku MELE, kamu sekarang sudah bisa membuat sticker sendiri. oh iya, jangan lupa ya join juga channel saya. [disini](https://t.me/DODEKATHEONS3)
...
def kang(bot: Bot, update: Update, args: List[str]):
    ...
    if msg.reply_to_message:
        ...
        try:
            ...
            msg.reply_text(f"Sticker Berhasil ditambahkan, silahkan cek disini cok > [pack](t.me/addstickers/{packname})" +
            ...
            if (
                ...
                msg.reply_text(
                    (
                        (
                            (
                                f"Sticker Berhasil ditambahkan, silahkan cek disini cok > [pack](t.me/addstickers/{packname})"
                                + "\n"
                                "Emoji is:"
                            )
                            + " "
    ...
    if args:
        ...
        try:
            ...
            if (
                ...
                bot.add_sticker_to_set(user_id=user.id, name=packname,
                                        png_sticker=open('kangsticker.png', 'rb'), emojis=sticker_emoji)
                msg.reply_text("Sticker Berhasil ditambahkan, silahkan cek disini cok > [pack](t.me/addstickers/%s)" % packname + "\n" +
                            "Emoji is:" + " " + sticker_emoji, parse_mode=ParseMode.MARKDOWN)
            elif e.message == "Stickers_too_much":
                msg.reply_text("Max packsize reached. Press F to pay respecc.")
            elif e.message == "Stickerset_invalid":
...
def makepack_internal(msg, user, png_sticker, emoji, bot, packname, packnum):
    ...
    try:
        ...
        success = bot.create_new_sticker_set(user.id, packname, f"{name}s | @DODEKATHEONS3" + extra_version,
